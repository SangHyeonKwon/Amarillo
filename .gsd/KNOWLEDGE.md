...
- **[S06 리뷰 R1 / Rule] 파괴적 정정의 안전성은 *실패 방향*과 *의존 가정*으로
  서술하라 — "보수적/안전" 라벨 금지**: S06-SUMMARY/D010/docs에 floor 롤백을
  "보수적 = 과삭제 후 재인덱싱 = 안전"이라 적었으나 실제는 **under-delete**
  (윈도우보다 깊은 reorg면 윈도우 밖 무효 블록 잔존 → 조용한 오염). 정확성은
  "reorg 깊이 ≤ scan window"라는 **미명시 가정**에 의존했고 그 가정 없이는 "안전"이
  거짓이었다. 리뷰가 *내가 쓴 문서*에서 이 역방향 서술을 잡아냄(2026-05-20 정정).
  규칙: 데이터 손실/정정 로직은 ① 틀릴 때 **over냐 under냐**(방향), ② 정확성이
  기대는 **가정**, ③ 그 가정이 깨지는 조건과 실무 확률을 명시한다. "보수적"·"안전"
  같은 결론 라벨만 적지 않는다. 자기 산출물도 홀리스틱 리뷰 대상(정직성 > 일관성).
- **[S06 리뷰 R5 / 상속 리스크, 신규 결함 아님] 마이그레이션 `BEGIN/COMMIT`
  중첩**: `migrations/*.sql`의 명시적 `BEGIN;…COMMIT;`는 sqlx 마이그레이터가
  각 마이그레이션을 감싸는 자체 트랜잭션과 **중첩**된다. Postgres가 허용하고
