...
- [ ] **S18 — 프론트 `/alerts` + M006 마감** · risk: low · **READY** (sketch 해제 — S17 완료로 인프라·예시·docs 정렬)
  - **T01 — 키 입력 UI (세션 메모리)**:
    - `web/src/pages/alerts/` 페이지 상단에 `<ApiKeyInput />` 컴포넌트:
      세션 메모리(React state)에만 저장, localStorage *미저장* (XSS 표면 최소화).
      placeholder/redacted 입력(`type="password"` 또는 직접 마스킹) + "키 적용"
      버튼. 키 적용 시 페이지 새로고침해도 *세션 비웠으면 다시 입력 필요* — 의도된
      운영 안전 시그널.
    - 키가 *없는* 상태 = write 버튼(생성 / 회전 / 비활성) 모두 *disabled* + 안내
      배너 ("API 키를 입력하면 활성화됩니다 — S16/M006").
    - 환경 변수 fallback도 *없음* — `NEXT_PUBLIC_*` 등 빌드 타임 주입은 *번들에
      박힘*이라 XSS/소스맵 노출 표면. 사용자가 *런타임에 직접 입력*.
  - **T02 — 401 응답 처리 + write 자동 헤더 부착**:
    - `web/src/api/client.ts` (또는 동등 위치)에 `apiKey` 주입 패턴. write
      메서드(POST/DELETE/rotate)에만 `Authorization: Bearer` 자동 부착, GET은
      무부착.
    - 401 응답 시 *명확한 에러 메시지* ("Unauthorized — check API key. The key
      is loaded into session memory only and clears on refresh."). 키 입력 폼
      쪽으로 운영자 시선 유도.
    - 키 입력 후 *기존 401 에러 dismissible* + 재시도 가능.
  - **T03 — M006 마감 cookbook 갱신 + ROADMAP M006 ✅ SHIPPED + M006-SUMMARY**:
    - `docs/cookbook.md`에 프론트 사용법 한 단락 추가 또는 신규 섹션 "From the
      `/alerts` page" — 키 입력 → 구독 생성 → 회전 → 비활성 step-by-step.
    - `.gsd/M006-SUMMARY.md` 신설 (M005-SUMMARY 패턴) — 세 슬라이스(S16/S17/S18)
      통합 + 두 페르소나(dApp 개발자 / 봇 운영자) + 새 페르소나(*운영자*)에
      대한 완결 흐름.
    - ROADMAP M006 헤더 → ✅ SHIPPED → M006-SUMMARY.md, S18 → `[x] DONE`.
    - BACKLOG.md 우선순위 표 갱신 — M006 마감 후 다음 후보 정렬.
  - 게이트: web typecheck + test (29 + 신규) + build / cargo workspace 무회귀
    (코드 0 변경 자동) / verify 3종 무회귀 (코드 0 변경 자동).

> M006 출하 = S16 ∧ S17 ∧ S18. **S16 ✅ → S17 ✅ → S18 진입 가능**. 마지막
> 슬라이스(S18)는 프론트만 변경 — 게이트 부담 작음. S18 출하 시 M006 ✅ SHIPPED.
